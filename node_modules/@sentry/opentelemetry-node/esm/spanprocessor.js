...
import { isSentryRequestSpan } from './utils/is-sentry-request.js';
import { mapOtelStatus } from './utils/map-otel-status.js';
import { parseSpanDescription } from './utils/parse-otel-span-description.js';
...
class SentrySpanProcessor  {
   ...
   */
   onStart(otelSpan, parentContext) {
    ...
    if (!hub) {
      (typeof __SENTRY_DEBUG__ === 'undefined' || __SENTRY_DEBUG__) && logger.error('SentrySpanProcessor has triggered onStart before a hub has been setup.');
      return;
    }
    const scope = hub.getScope();
    if (!scope) {
      (typeof __SENTRY_DEBUG__ === 'undefined' || __SENTRY_DEBUG__) && logger.error('SentrySpanProcessor has triggered onStart before a scope has been setup.');
      return;
    }
   ...
   */
   onEnd(otelSpan) {
    const hub = getCurrentHub();
    if (!hub) {
      (typeof __SENTRY_DEBUG__ === 'undefined' || __SENTRY_DEBUG__) && logger.error('SentrySpanProcessor has triggered onEnd before a hub has been setup.');
      return;
    }
...
export { SENTRY_SPAN_PROCESSOR_MAP, SentrySpanProcessor };
